...
import { createStreamSchema, CreateStreamInput } from "../schema/stream.schema";
import { HASHTAG_REGEX } from "@/constants/regex";
import { useCreateStreamMutation } from "../hooks/useCreateStream";
...
export const CreateStreamForm = memo(
  ({ onCancel, onSubmittingChange }: CreateStreamFormProps) => {
    const fileInputRef = useRef<HTMLInputElement>(null);
    const [previewUrl, setPreviewUrl] = useState<string | null>(null);

    const inputRef = useRef<HTMLInputElement | null>(null);
    const textareaRef = useRef<HTMLTextAreaElement | null>(null);
    const { mutate: createStream, isPending } = useCreateStreamMutation();
    const router = useRouter();

    // Notify parent (modal) about loading state
    useEffect(() => {
    ...
    const onSubmit = (data: CreateStreamInput) => {
      ...
      const allTags = Array.from(new Set([...tagsFromTitle, ...tagsFromDesc]));

      const rawData = { ...data, extractedTags: allTags };
      const formData = toFormData(rawData);

      createStream(formData, {
        onSuccess: (data) => {
          router.push(`/stream/${data.streamID}/setup`);
        },
        onError: (error) => {
      ...
      <div className="w-full relative">
        ...
        <form className="space-y-6" onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="space-y-2">
            ...
            <Input
              variant="surface"
              placeholder="Enter your stream title (e.g. Playing #CyberArena)"
              enableHashtags
              {...titleRest}
              value={titleVal}
              ref={handleTitleRef}
          ...
          <div className="space-y-2">
            ...
            <Textarea
              variant="surface"
              placeholder="Enter your stream description (e.g. Playing #CyberArena)"
              rows={4}
              enableHashtags
              {...descRest}
              value={descVal}
...
CreateStreamForm.displayName = "CreateStreamForm";
